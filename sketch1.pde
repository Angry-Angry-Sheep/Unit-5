// Ball variables
float ballX, ballY;
float ballSpeedX = 4;
float ballSpeedY = 3;
float ballSize = 20;
float maxSpeed = 12;

// Paddle variables
float paddleWidth = 15;
float paddleHeight = 55;

// Player 1 paddle
float p1X, p1Y;

// Player 2 paddle
float p2X, p2Y;

// Score
int p1Score = 0;
int p2Score = 0;

// Bricks
int brickCols = 3;
int brickRows;
float brickW = 20;
float brickH = 20;
boolean[][] p1Bricks;
boolean[][] p2Bricks;
float brickMargin = 30;

// Track pressed keys
int player1Movement = 0;
int player2Movement = 0;

// Circles
float circleY1 = 0;
float circleY2 = 0;
float circleSpeed1 = 2;
float circleSpeed2 = 2;
float circleSize = 40;

// Bullets
ArrayList<Bullet> bullets = new ArrayList<Bullet>();

void setup() {
  size(800, 400);
  resetBall();
  
  // Set paddle positions
  p1X = brickMargin + brickCols * brickW + 10;
  p2X = width - brickMargin - brickCols * brickW - 10 - paddleWidth;
  p1Y = height / 2 - paddleHeight / 2;
  p2Y = height / 2 - paddleHeight / 2;

  // Initialize bricks
  brickRows = int(height / brickH);
  p1Bricks = new boolean[brickCols][brickRows];
  p2Bricks = new boolean[brickCols][brickRows];

  for (int i = 0; i < brickCols; i++) {
    for (int j = 0; j < brickRows; j++) {
      p1Bricks[i][j] = true;
      p2Bricks[i][j] = true;
    }
  }
}

void draw() {
  background(0);
  
  // Move and draw bouncing circles
  circleY1 += circleSpeed1;
  circleY2 += circleSpeed2;

  if (circleY1 < 0 || circleY1 > height) circleSpeed1 *= -1;
  if (circleY2 < 0 || circleY2 > height) circleSpeed2 *= -1;

  // Draw bricks
  drawBricks(p1Bricks, brickMargin);
  drawBricks(p2Bricks, width - brickMargin - brickCols * brickW);

  // Draw the bouncing circles behind the brick wall
  noStroke();
  fill(255, 0, 0, 100);
  ellipse(brickMargin + brickCols * brickW / 2, circleY1, circleSize, circleSize);

  fill(0, 0, 255, 100);
  ellipse(width - brickMargin - brickCols * brickW + brickCols * brickW / 2, circleY2, circleSize, circleSize);

  // Draw the ball
  fill(255);
  ellipse(ballX, ballY, ballSize, ballSize);

  // Draw paddles
  fill(255);
  rect(p1X, p1Y, paddleWidth, paddleHeight);
  rect(p2X, p2Y, paddleWidth, paddleHeight);

  // Draw score
  textSize(32);
  textAlign(CENTER, TOP);
  text(p1Score + " : " + p2Score, width / 2, 10);

  // Move ball
  ballX += ballSpeedX;
  ballY += ballSpeedY;

  // Bounce off top and bottom
  if (ballY <= 0 || ballY >= height) {
    ballSpeedY *= -1;
  }

  // Move paddles based on keys
  if (player1Movement == 1) p1Y -= 7.5;
  if (player1Movement == -1) p1Y += 7.5;
  if (player2Movement == 1) p2Y -= 7.5;
  if (player2Movement == -1) p2Y += 7.5;

  // Clamp paddles to screen
  p1Y = constrain(p1Y, 0, height - paddleHeight);
  p2Y = constrain(p2Y, 0, height - paddleHeight);

  // Paddle collision
  if (ballX - ballSize / 2 < p1X + paddleWidth && ballX > p1X &&
      ballY > p1Y && ballY < p1Y + paddleHeight) {
    ballSpeedX *= -1.1;
    ballSpeedY -= player1Movement * 2;
    ballSpeedY += random(-0.15, 0.15);
    ballX = p1X + paddleWidth + ballSize / 2;
    clampBallSpeed();
  }

  if (ballX + ballSize / 2 > p2X && ballX < p2X + paddleWidth &&
      ballY > p2Y && ballY < p2Y + paddleHeight) {
    ballSpeedX *= -1.1;
    ballSpeedY -= player2Movement * 2;
    ballSpeedY += random(-0.15, 0.15);
    ballX = p2X - ballSize / 2;
    clampBallSpeed();
  }

  // Check brick collisions
  checkBrickCollision();

  // Wall bounce
  if (ballX - ballSize / 2 < 0) {
    ballX = ballSize / 2;
    ballSpeedX *= -1;
    clampBallSpeed();
  }
  if (ballX + ballSize / 2 > width) {
    ballX = width - ballSize / 2;
    ballSpeedX *= -1;
    clampBallSpeed();
  }

  // Game over check
  if (ballSpeedX == 999) {
    noLoop();
    textSize(40);
    fill(255, 0, 0);
    textAlign(CENTER, CENTER);
    text("Game Over", width / 2, height / 2);
  }

  // Draw and handle bullet logic
  for (int i = bullets.size() - 1; i >= 0; i--) {
    Bullet b = bullets.get(i);
    b.update();
    b.display();
    if (b.isOffScreen()) {
      bullets.remove(i);
    }
  }
}

// Draw bricks for a player
void drawBricks(boolean[][] bricks, float startX) {
  for (int i = 0; i < brickCols; i++) {
    for (int j = 0; j < brickRows; j++) {
      if (bricks[i][j]) {
        float x = startX + i * brickW;
        float y = j * brickH;
        fill(200, 200, 200);
        rect(x, y, brickW, brickH);
      }
    }
  }
}

// Handle ball and brick collisions
void checkBrickCollision() {
  float p1StartX = brickMargin;
  for (int i = 0; i < brickCols; i++) {
    for (int j = 0; j < brickRows; j++) {
      if (p1Bricks[i][j]) {
        float x = p1StartX + i * brickW;
        float y = j * brickH;
        if (ballX + ballSize / 2 > x &&
            ballX - ballSize / 2 < x + brickW &&
            ballY + ballSize / 2 > y &&
            ballY - ballSize / 2 < y + brickH) {
          p1Bricks[i][j] = false;
          ballSpeedX *= -1.1;
          clampBallSpeed();
          return;
        }
      }
    }
  }

  float p2StartX = width - brickMargin - brickCols * brickW;
  for (int i = 0; i < brickCols; i++) {
    for (int j = 0; j < brickRows; j++) {
      if (p2Bricks[i][j]) {
        float x = p2StartX + i * brickW;
        float y = j * brickH;
        if (ballX + ballSize / 2 > x &&
            ballX - ballSize / 2 < x + brickW &&
            ballY + ballSize / 2 > y &&
            ballY - ballSize / 2 < y + brickH) {
          p2Bricks[i][j] = false;
          ballSpeedX *= -1.1;
          clampBallSpeed();
          return;
        }
      }
    }
  }
}

// Limit the ball speed to maxSpeed
void clampBallSpeed() {
  float speedMag = dist(0, 0, ballSpeedX, ballSpeedY);
  if (speedMag > maxSpeed) {
    float angle = atan2(ballSpeedY, ballSpeedX);
    ballSpeedX = cos(angle) * maxSpeed;
    ballSpeedY = sin(angle) * maxSpeed;
  }
}

// Reset ball to center
void resetBall() {
  ballX = width / 2;
  ballY = height / 2;
  ballSpeedX = random(1) > 0.5 ? 5 : -5;
  ballSpeedY = random(-3, 3);
}

// Track key presses
void keyPressed() {
  if (key == 'w' || key == 'W') player1Movement = 1;
  if (key == 's' || key == 'S') player1Movement = -1;
  if (keyCode == UP) player2Movement = 1;
  if (keyCode == DOWN) player2Movement = -1;
}

void keyReleased() {
  if (key == 'w' || key == 'W' || key == 's' || key == 'S') player1Movement = 0;
  if (keyCode == UP || keyCode == DOWN) player2Movement = 0;
}

class Bullet {
  float x, y, speed;
  int direction;
  
  Bullet(float startX, float startY, float speed, int direction) {
    this.x = startX;
    this.y = startY;
    this.speed = speed;
    this.direction = direction;
  }
  
  void update() {
    x += speed * direction;
  }
  
  void display() {
    fill(255, 255, 0);
    ellipse(x, y, 10, 10);
  }
  
  boolean isOffScreen() {
    return x < 0 || x > width;
  }
}
