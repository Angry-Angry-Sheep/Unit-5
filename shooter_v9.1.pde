...
void draw() {
  ...
  if (!gameStarted && !inStore) {
    ...
    // Get Pulse Ability
    if (isMouseOver(width/2, 240, 180, 30) && money >= 50 && mousePressed && !clickConsumed && !pulseAbility) {
      money -= 50;
      clickConsumed = true;
      pulseAbility = true;
    }
    ...
    // Get Laser Module
    if (isMouseOver(width/2, 320, 180, 30) && money >= 50 && mousePressed && !clickConsumed && !laserAbility) {
      money -= 50;
      clickConsumed = true;
      laserAbility = true;
    }
