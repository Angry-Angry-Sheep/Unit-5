...
// === Laser Settings ===
int laserDamage = 0;    // if 0, laser is disabled; higher → more HP removed per frame
...
if (laserDamage > 0) {
  ...
  if (blocker != null) {
    blocker.hp -= laserDamage / (frameRate * 2);
    if (blocker.hp <= 0) {
...
void keyPressed() {
  ...
  if (upgradeMenu) {
    ...
    collectableCount = 0;
    collectableGoal += 2;
    enemySpawnCount = max(20, enemySpawnCount - 6);
    upgradeMenu = false;
    paused = false;
    return;
  }
