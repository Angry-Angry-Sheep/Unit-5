...
if (boss != null) {
  ...
  // now, test bullet collisions _before_ you display
  for (int i = bullets.size()-1; i >= 0; i--) {
    Bullet b = bullets.get(i);
...
void drawStrikethroughText(String txt, float x, float y) {
  ...
  // Because y is the TOP of the text box (textAlign TOP), 
  float lineY = y;
...
class Boss extends Enemy {
  // initial patrol speed
  float maxSpeed = 4;
  // how much the boss jumps its velocity when a shot is nearby
  float dodgeForce  = 750;   
  // how close a bullet must be to trigger dodge
  float dodgeRadius = 120;
  
  int shootTimer = 0;
  int shootInterval = 160;
  int spreadCount = 25;    // bullets per explosion ring
  float bulletSpeed = 4.0;
  
  float spawnRadius = 60;

  Boss(float x, float y) {
    super(x, y, 200, veryAdvancedEnemyImg);
  ...
  void update() {
    ...
    // bounce on the left/right walls
    if (x < 50 || x > width - 50) {
      vx *= -1;
      x = constrain(x, 50, width - 50);
    }
    // bounce on the top/half‐screen boundary
    if (y < 50 || y > height/2) {
      vy *= -1;
    ...
    for (Bullet b : bullets) {
      ...
      if (d2 < dodgeRadius*dodgeRadius && d2 > 0) {
        float d = sqrt(d2);
        // unit vector pointing _away_ from the bullet
        float ux = dx / d;
        float uy = dy / d;
        // impulse scaled by how close it is (closer → bigger)
        float impulse = dodgeForce / (d + 10);
        vx += ux * impulse * (1.0/frameRate);
        vy += uy * impulse * (1.0/frameRate);
      }
    }
 ...
 @Override
  void shoot() {
    ...
    for (int i = 0; i < spreadCount; i++) {
      float angle = TWO_PI * i / spreadCount;
      // compute an offset spawn position on a circle:
      float bx = x + cos(angle) * spawnRadius;
      float by = y + sin(angle) * spawnRadius;
      float vx_b = cos(angle) * bulletSpeed;
      float vy_b = sin(angle) * bulletSpeed;
      bullets.add(new Bullet(bx, by, vx_b, vy_b, veryAdvEnemyBulletImg, this));
    }
  }
  ...
  void display() {
    ...
    float bossScale = 1.3;
    image(img, x, y,
          img.width * SCALE * bossScale,
          img.height * SCALE * bossScale);
  }
}
